...
def construct_pointwise_data(
    ...
    with open(epoch_filename, "w") as g:
        ...
        for doc_idx in tqdm(chunk_indexs):
            ...
            pos_segment_ids = [0 for _ in range(len(pos_post) + 2)] + [
                1 for _ in range(len(pos_resp) + 1)
            ]
            neg_tokens = ["[CLS]"] + neg_post + ["[SEP]"] + neg_resp + ["[SEP]"]
            neg_segment_ids = [0 for _ in range(len(neg_post) + 2)] + [
                1 for _ in range(len(neg_resp) + 1)
            ]
            (
                pos_tokens,
                pos_masked_lm_positions,
                pos_masked_lm_labels,
            ...
            (
                neg_tokens,
                neg_masked_lm_positions,
                neg_masked_lm_labels,
            ) = create_masked_lm_predictions(
                neg_tokens,
                masked_lm_prob,
                max_predictions_per_seq,
            ...
            pos_tokens_idx_labels = bert_tokenizer.convert_tokens_to_ids(
                pos_masked_lm_labels
            )
            neg_tokens_idx = bert_tokenizer.convert_tokens_to_ids(neg_tokens)
            neg_tokens_idx_labels = bert_tokenizer.convert_tokens_to_ids(
                neg_masked_lm_labels
            )
            pos_instance = {
...
def get_layer_info(nodes, max_layer_num, node_max_length, seq_max_length):
    ...
    while len(node_queue) > 0:
        if (
            node_queue[0]["depth"] != last_depth
            and node_queue[0]["depth"] <= max_layer_num
        ):
        ...
        if (
            node_queue[0]["name"] == "textnode"
            or node_queue[0]["depth"] == max_layer_num
        ):
            text_info = {}
            ...
            for child in children:
                if (
                    nodes[child]["name"] == "textnode"
                    and nodes[child]["text"].strip() == ""
                ):
                    continue
                else:
            ...
            waiting_mask = (
                [0] * 2
                + [1] * children_num
                + [0] * max(node_max_length - (children_num + 2), 0)
            )
...
def construct_pointwise_examples(
    examples,
    chunk_indexs,
    max_seq_len,
    max_tag_len,
    ...
    for doc_idx in tqdm(chunk_indexs):
        ...
        layer_info = get_layer_info(
            ...
            node_max_length=max_tag_len,
            seq_max_length=max_seq_len,
        )
