...
class Webformer(FraBert):
    ...
    def get_task_embeddings(
        self, embeddings, negetive_embeddings, mask_embeddings, waiting_mask
    ):
        ...
        for i in range(batch_size):
            for j in range(tag_len):
                ...
                if waiting_mask[i, j, temp[0]]:
                    temp_embedding = self.sample_one_embedding(
                        waiting_mask, negetive_embeddings, i, j
                    )
    ...
    def forward(
        ...
        mnp_loss_fct = torch.nn.CosineEmbeddingLoss(
            margin=0.0, size_average=None, reduction="mean"
        )
        ...
        for layer in range(self.layer_num):
            ...
            if not len(torch.nonzero(node_layer_mask, as_tuple=False)) and not len(
                ...
                continue
            layer_position = position[:, cur_layer - 1, :].view(
                batch_size, -1
            )  # (b,l_num,l_sum_num)
            if len(output) != 0:
            ...
            if len(torch.nonzero(node_layer_mask, as_tuple=False)):
                layer_waiting_mask = waiting_mask[node_layer_mask].type(
                    torch.bool
                )  # (b,lay_n_num,n_len)
                layer_node_input_ids = node_input_ids[
                    node_layer_mask
                ]  # (b,lay_n_num,n_len)
                layer_node_embeds = self.bert.embeddings.word_embeddings(
                    layer_node_input_ids
                )  # (b,lay_n_num,n_len,h_dim)
                if len(output) != 0:
                ...
                node_output = layer_node_output[:, 0, :].view(
                    -1, self.config.hidden_size
                )
            if len(torch.nonzero(token_layer_mask, as_tuple=False)):
                layer_token_input_ids = token_input_ids[
                    token_layer_mask
                ]  # (b,lay_seq_num,seq_len)
                layer_inputs_type_idx = inputs_type_idx[
                    token_layer_mask
                ]  # (b,lay_n_num,n_len)
                layer_token_embeds = self.bert.embeddings.word_embeddings(
                    layer_token_input_ids
                )  # (b,lay_seq_num,seq_len,h_dim)
                layer_token_embeds += self.type_embedding(
                    layer_inputs_type_idx
                )  # (b,lay_seq_num,seq_len,h_dim)
                ...
                if len(word_logits):
                    word_logits = torch.cat(
                        (
                            word_logits,
                            self.cls(layer_text_output).view(
                                -1, self.config.vocab_size
                            ),
                        ),
                        dim=0,
                    )
                    new_token_labels = torch.cat(
                        (new_token_labels, layer_toten_labels), dim=0
                    )
                else:
                    word_logits = self.cls(layer_text_output).view(
                        -1, self.config.vocab_size
                    )
                    new_token_labels = layer_toten_labels
                text_output = layer_text_output[:, 0, :].view(
                    -1, self.config.hidden_size
                )
        ...
        all_embeddings, neg_embeddings, mask_embeddings = self.get_embedding(
            output, waiting_mask, node_input_ids
        )
        (
            train_embeddings,
            negetive_embeddings,
        ...
        order_labels = (
            torch.max(order_labels, dim=0).values - order_labels
        ).true_divide(torch.sum(order_labels, dim=0))
        ...
        pcm_pos_score = self.masked_item_prediction(pcm_parent, pcm_positive).view(
            1, -1
        )
        pcm_neg_score = self.masked_item_prediction(pcm_parent, pcm_negetive).view(
            1, -1
        )
        pcm_scores = torch.cat((pcm_pos_score, pcm_neg_score), dim=0).permute(1, 0)

        snm_loss = snm_loss_fct(
            snm_scores, torch.ones(snm_pos_score.size(-1)).to(device).long()
        )
        pcm_loss = pcm_loss_fct(
            pcm_scores, torch.ones(pcm_pos_score.size(-1)).to(device).long()
        )
